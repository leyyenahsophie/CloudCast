...
def weather_view(request):
    ...
    if request.method == "POST":
        ...
        if city:
            azure_function_url = f"https://cloudcast001.azurewebsites.net/api/cloudcast?code=BVB8LkZemSLXqXPlymD8aPodHCKnvpKjHnoLmATXEgG7AzFu7cXpCQ%3D%3D&={city}"
...
def weather_api_view(request):
    try:
        weather_data = WeatherData.objects.latest('created_at')
        return JsonResponse({
